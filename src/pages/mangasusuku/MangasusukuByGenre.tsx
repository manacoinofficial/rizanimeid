...
const MangasusukuByGenre = () => {
  const { genreId } = useParams<{ genreId: string }>();
  const [page, setPage] = useState(1);

  const { data, isLoading } = useQuery({
    queryKey: ['mangasusuku-genre', genreId, page],
    queryFn: () => mangasusukuApi.getByGenre(genreId!, page),
    enabled: !!genreId,
  });
...
export default MangasusukuByGenre;
